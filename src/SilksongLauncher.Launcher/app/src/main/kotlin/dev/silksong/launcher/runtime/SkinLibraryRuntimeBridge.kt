...
object SkinLibraryRuntimeBridge {
    ...
    internal fun initialize(context: Context, profileId: String) = synchronized(this) {
        ...
        if (profileId == HollowKnightProfile.id) factory = {
            SkinLibraryRuntimeAccess(SkinLibraryStore.production(context, HollowKnightProfile))
        }
    }
    @JvmStatic fun readConfiguration(): String {
        try { GameProcessStartup.requireProfile(HollowKnightProfile.id) }
        catch (error: Exception) { return failure("PROFILE_REJECTED", error.message.orEmpty()) }
        return try {
            val current = access ?: requireNotNull(factory) { "Hollow Knight skin runtime is not initialized" }.invoke().also { access = it }
    ...
    @JvmStatic fun reportResult(configSha256: String, activePackId: String, activeTreeSha256: String, status: String, detail: String): Boolean =
        try {
            GameProcessStartup.requireProfile(HollowKnightProfile.id)
    ...
    fun readConfiguration(): String {
        val result = store.locked(nonblocking = true) {
            ...
            val wire = JsonObject().apply {
                addProperty("ok", true); addProperty("profileId", SkinLibraryCodec.PROFILE)
            ...
            if (document.mode != LibraryMode.OFF) {
                ...
                val game = requireNotNull(verifiedManifest).games.getValue(SkinLibraryCodec.PROFILE)
