...
rather than loaded by it, so installing or disabling a mod means rebuilding.
Config files are read at startup, so those you can change freely.

1. Put plugin DLLs in `Android/data/dev.silksong.launcher/files/mods`.
