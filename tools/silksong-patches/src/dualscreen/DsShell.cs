...
public class DsShell
{
    ...
    {
        if (_disposed || idle == _idle) return;
        _idle = idle;
        _gestures.Reset();
    ...
    /// <summary>
    /// Stops native page/HUD ownership before a scene unload while retaining
    /// the shell and its selected tab for the next scene.
    /// </summary>
    public void SetTransitioning(bool transitioning)
    {
        if (_disposed || transitioning == _transitioning) return;
        if (transitioning)
        {
            EndSlide();
            _actions.Clear();
    ...
    /// <summary>Immediately releases any native HUD render scope.</summary>
    ...
    void UpdateOperational()
    {
        ...
        if (!e.Broken)
            Guard(e, () => { if (operational) e.Screen.OnShow(); else e.Screen.OnHide(); });
    }
    ...
    public void Show(int index)
    {
        ...
        if (_operational) Guard(e, () => e.Screen.OnShow());
        // The outgoing screen's actions are not the incoming one's.
        _actions.Clear();

        bool canSlide = _operational && _slideSeconds > 0f &&
                        from >= 0 && from < _entries.Count &&
