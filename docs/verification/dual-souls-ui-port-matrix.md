...
| Shared contract extraction | `NOT_STARTED` | Depends on the accepted Hollow Knight reference | None | Existing shared tweak contracts are useful infrastructure but are not a complete companion/skin abstraction |
...
| Stage8/Mods | Task100 | Process-owned shared controller/store/session and reachable native-style gear/modal; default-off, restoration/retry and profile isolation tests; unavailable capabilities remain explicit |
| Shared Silksong skins/death rotation | Task101 | Closed per-game catalog/profile authority, actual SS texture/death bindings, reused apply/restore/rotation algorithms and cross-game tests |
...
and scenery consumption, plus overlay → future Mods callback → cached tab
consumption. `SetModsGestureConsumer` is the concrete Task100 seam, not Mods UI.
The selected-page consumer is still absent. `DsPortSelectState` has executable
identity, read-only-display/action separation, action-time legality and reentrant
selection-revision checks, but no native selection/equip adapter.
