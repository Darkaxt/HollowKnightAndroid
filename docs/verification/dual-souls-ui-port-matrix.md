...
| Shared Silksong skins/death rotation | Task101 — `HOST-COMPLETE / DEVICE-DEFERRED` at `a701d33` | Closed 11-target Silksong catalog/profile; exact multi-instance tk2d collection/material admission across default/active/windy/crest and persistent HUD ownership; character-only ROTATE; persistent HUD ON; complete sparse-target omissions; retry-owned teardown; bounded ordered per-occurrence death ownership; and exact pinned post-normalization `HeroController.<Die>d__1101.MoveNext` classification. The managed bridge commits hero/manager/token into a 32-slot ring before publishing each occurrence, preserving distinct owners across counter jumps. Owner replacement cancels only the stale occurrence through the exact JNI/store path and retains valid newer work; overflow fails closed. BundleSurgery self-verifies the fresh canonical SHA/IL before publication, and the launcher independently checks the pinned SHA and runs canonical verification before replacing staged `Assembly-CSharp.dll`. The host checkpoint passes 1,569 shared, 101 launcher, 148 Python, and 39 BundleSurgery tests; compiles 82 Silksong sources against exact 1.0.29980 and preserves the exact Hollow Knight compile. The death rewrite accepts only Assembly-CSharp SHA-256 `1af095416b89f73993058f9cbac3a93959d928314b735cc4acbca7bf1a952d2d`, produces canonical SHA-256 `86e8ffd402bb5e58c57d89ef2e0c3fa8dd3e89a9c1c049d4bf663484434b6a8e`, is byte-idempotent, and rejects original or already-bridged unrelated drift without output. Android bundle identity/parity, sampled material appearance, Scythe/Shaman shader behavior, crest/fallback visuals, HUD effects, resource pressure, death/respawn visuals/lifecycle, and persistence interaction remain device-only. Evidence: `docs/verification/evidence/task101-spec-fix-a701d33/` (supersedes `task101-spec-fix-bcf3508`) |
