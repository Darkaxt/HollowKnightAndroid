...
| 3 — second-screen modal | HOST-COMPLETE | The game-neutral layout pins the Dual Souls HUD/frame/bottom-tab/gear structure; the Silksong adapter consumes it, keeps Mods out of the tab list, preserves modal containment, master/reset/error behavior, retains a prior working page after adapter failure, retries late fonts/art, and uses the shared touch minimum | Physical lower-panel interaction remains a Stage 4 device gate | Stage 5 must replace localized hard-coded labels and add focus/pressed/reduced-motion states; Stage 6 may replace deterministic ornaments after resident sprites load. Acceptance: language switching updates all HUD/modal labels; focus/press states pass keyboard/gamepad/touch review with motion both enabled and reduced; late-loaded art replaces fallback without geometry movement | 9/9 profile contracts; 29/29 shared tests including five layout/hit-test contracts; Android host 120 tests with zero failures/errors and two environment skips; 38/38 bundle-surgery tests; both exact profile patch compiles; debug AAR assembly |
| 4 — device proof | IN-PROGRESS | Fork-signed launcher, exact Silksong `1.0.29980` atomic on-device build, separate ARM64 Unity process, gameplay, display-1 activation, redesigned-shell visual proof, and first-run master OFF are proved | Physical gear/tab/modal touch, all four effects, process-exit persistence, post-audit signed rebuild, and Hollow Knight isolation remain | None | `docs/verification/silksong-device-2026-08-31.md` |
